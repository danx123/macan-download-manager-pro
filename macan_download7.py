...
# --- TAMBAHAN --- Ikon baru untuk Stop All dan Search
...
SVG_ABOUT = """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>""" # <<< BARU: Ikon untuk tombol About
...
# --- ENUM & Data Class untuk Model (Tetap sama) ---
...
# --- TAMBAHAN --- Custom Proxy Model untuk filter ganda (status & nama file)
class CustomFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        # Ambil index untuk kolom nama (0) dan status (3)
...
class DownloadManager(QObject):
    ...
    def control_download(self, uid, action):
        ...
        if action == 'pause' and item.worker and item.status == DownloadStatus.DOWNLOADING: item.worker.toggle_pause()
            ...
            else: # Jika masih di queue, hapus dari queue
    ...
    def clear_completed_downloads(self):
        """Menghapus semua item yang sudah tidak aktif (Finished, Error, Stopped) dari daftar."""
        # Buat daftar baru yang hanya berisi unduhan yang masih aktif atau dalam antrian
        downloads_to_keep = [
        ...
        if len(downloads_to_keep) < len(self.downloads):
            ...
            # Kirim sinyal agar UI diperbarui
            self.model_updated.emit()

# --- UI/UX Lainnya (Tetap sama) ---
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # --- TAMBAHAN --- Aksi-aksi baru untuk toolbar
        ...
        #action_clear_completed = QAction(create_svg_icon(SVG_CLEAR_ALL, "red"), "Clear Completed", self)
        #action_clear_completed.triggered.connect(self.clear_completed_list)
        ...
        action_about = QAction(create_svg_icon(SVG_ABOUT), "About", self) # <<< BARU
        action_about.triggered.connect(self.show_about_dialog) # <<< BARU
        ...
        #toolbar.addAction(action_clear_completed)
        toolbar.addSeparator()
        toolbar.addAction(action_settings)
        toolbar.addAction(action_about) # <<< BARU: Menambahkan tombol About ke toolbar
        
        # --- TAMBAHAN --- Widget kosong untuk mendorong search bar ke kanan
        spacer = QWidget()
        spacer.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        toolbar.addWidget(spacer)

        # --- TAMBAHAN --- Kotak pencarian di toolbar
        ...
        # --- TAMBAHAN --- Menggunakan proxy model kustom
        ...
        # --- TAMBAHAN --- Koneksi signal untuk mengaktifkan/menonaktifkan tombol
    ...
    # --- TAMBAHAN --- Fungsi-fungsi baru untuk menangani aksi toolbar
    def get_selected_items(self):
        """Mengembalikan daftar objek DownloadItem yang dipilih di tabel."""
        items = []
    ...
    def pause_selected(self):
        ...
        for item in items:
            # Aksi Pause hanya berlaku untuk yang sedang downloading
            # Aksi Resume (kebalikannya) juga bisa dimasukkan di sini jika tombolnya toggle
    ...
    def clear_completed_list(self):
        """Menangani klik pada tombol 'Clear Completed'."""
        # Hitung dulu berapa item yang akan dihapus
        items_to_clear = [
    ...
    def on_table_double_clicked(self, proxy_index):
        """Membuka file yang sudah selesai di-download saat di-double-click."""
        if not proxy_index.isValid():
            return
            
        # Dapatkan item asli dari source model
        source_index = self.proxy_model.mapToSource(proxy_index)
        item = self.manager.downloads[source_index.row()]
        
        # Hanya buka jika statusnya FINISHED dan file-nya ada
        if item.status == DownloadStatus.FINISHED:
            if os.path.exists(item.filepath):
                try:
                    # Logika cross-platform untuk membuka file
                    if sys.platform == "win32":
                        os.startfile(item.filepath)
                    elif sys.platform == "darwin": # macOS
                        os.system(f'open "{item.filepath}"')
                    else: # linux
                        os.system(f'xdg-open "{item.filepath}"')
    ...
    def on_search_text_changed(self):
        """Slot untuk menangani perubahan teks pada kotak pencarian."""
        search_text = self.search_input.text()
        self.proxy_model.set_text_filter(search_text)

    @Slot()
    def update_toolbar_actions_state(self):
        """Memperbarui status enabled/disabled tombol toolbar berdasarkan item yang dipilih."""
        selected_items = self.get_selected_items()
        can_pause = False
        can_stop = False

        if selected_items:
            # Cek apakah ada item yang bisa di-pause/resume
            can_pause = any(item.status in [DownloadStatus.DOWNLOADING, DownloadStatus.PAUSED] for item in selected_items)
            # Cek apakah ada item yang bisa di-stop
    ...
    def create_context_menu(self, pos):
        ...
        # Pastikan functools.partial sudah di-import di atas file
        from functools import partial
        ...
        # --- SEMUA AKSI SEKARANG MENGGUNAKAN partial ---
        if item.status == DownloadStatus.DOWNLOADING:
    ...
    # <<< BARU: Fungsi untuk menampilkan dialog About
    ...
    def apply_stylesheet(self):
        # --- TAMBAHAN --- Sedikit penyesuaian untuk search bar di stylesheet
        self.setStyleSheet("""
            QMainWindow, QDialog { background-color: #262626; color: #FFFFFF; }
            QMessageBox { background-color: #2F2F2F; color: white; } /* <<< EDIT BARIS INI */
            ...
            QToolBar QLineEdit { padding: 4px; background-color: #FFFFFF; } /* Style khusus search bar */
